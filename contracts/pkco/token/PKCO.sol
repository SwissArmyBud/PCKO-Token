...
import "../../zeppelin/interfaces/IERC20.sol";
import "../../zeppelin/token/ERC20/extensions/IERC20Metadata.sol";

import "../../zeppelin/access/Ownable.sol";
import "../../zeppelin/utils/Context.sol";

import "./extensions/Charity.sol";
import "./extensions/Raffle.sol";
 ...
 */
contract PKCOToken is IERC20, IERC20Metadata, Ownable, Charity, Raffle {
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, uint256 _amount) {
        _name = name_;
        _symbol = symbol_;
    ...
    function _transfer(
        ...
        // _beforeTokenTransfer(sender, recipient, amount); // PKCO
        uint256 tokenomicsBalance = amount / 4; // PKCO
        _raffleBalance += tokenomicsBalance; // PKCO
        _charityBalance += tokenomicsBalance; // PKCO

        _balances[recipient] += amount - (3 * tokenomicsBalance);

        emit Transfer(sender, recipient, amount);

        // Replace transfer hook with raffle registration hook
        // _afterTokenTransfer(sender, recipient, amount); // PKCO
        _registerForRaffle(recipient); // PKCO
    }
    ...
    */
    ...
    function claimCharity() public override {
        _balances[_charityAddress] += _charityBalance;
        _charityBalance = 0;
    }
    ...
    */
    ...
    function _claimRaffle(uint256 entry, address account) internal override {
        ...
        if( !isEligible(raffle._raffleHash, account) ){ return; }
